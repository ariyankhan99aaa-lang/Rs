...
class RawButton:
    ...
    def to_dict(self):
        ...
        if 'icon_custom_emoji_id' in data:
            val = data['icon_custom_emoji_id']
            if val and str(val).isdigit():
                data['icon_custom_emoji_id'] = int(val)
            else:
...
FLAG_IDS = {
    ...
    'TO': '5431573849164048083', 'TT': '5434145026168699543', 'TN': '5911332947419468671',
...
SERVICE_IDS = {
    ...
    'TELEGRAM': '5267139126339084336',
    'INSTAGRAM': '5319160079465857105',
    'TIKTOK': '5327982530702359565',
    'FACEBOOK': '5269492171416832604',
    'GOOGLE': '5330237710655306682',
...
async def send_welcome(message):
    ...
    def p_btn(text, icon_id, style):
        d = {"text": text, "style": style}
        if icon_id and str(icon_id).isdigit():
            d["icon_custom_emoji_id"] = int(icon_id)
        return d

    rows = [
        [p_btn("Get Number", "6147489559526513603", "success"), p_btn("Stock Status", "5452069934089641166", "success")],
        [p_btn("Download OTP", "5255741073409452371", "success"), p_btn("Live Traffic", "5264919878082509254", "success")]
    ]
    
    if is_adm:
        rows.append([p_btn("My Balance", "6190336264940559752", "success"), p_btn("Admin Panel", "5341715473882955310", "primary")])
        rows.append([p_btn("Withdraw", "5255981634527704754", "success")])
        if is_dev:
            rows.append([p_btn("Bot Developer", "5314391089514291948", "primary")])
    else:
        rows.append([p_btn("My Balance", "6190336264940559752", "success"), p_btn("Withdraw", "6276028580551987901", "success")])
    ...
    welcome_text = (
        "<tg-emoji emoji-id=\"5310170944843579391\">🌟</tg-emoji> <b>Welcome to SMART Bot!</b> <tg-emoji emoji-id=\"5310170944843579391\">🌟</tg-emoji>\n\n"
        "<tg-emoji emoji-id=\"5240118799885158175\">💬</tg-emoji> <i>Main Menu</i>\n\n"
        "<tg-emoji emoji-id=\"5206607081334906820\">👇</tg-emoji> <b>Please select an option below:</b>"
    )
...
async def admin_panel_callback(call_or_msg):
    ...
    def a_btn(text, icon_id, callback, style="primary"):
        d = {"text": f"      {text}", "callback_data": callback, "style": style}
        if icon_id and str(icon_id).isdigit():
            d["icon_custom_emoji_id"] = int(icon_id)
        return d

    # Row 1: Country Management
    markup_list.append([a_btn("Country Management", "6188045471118790922", "admin_country_mgmt", "primary")])
    
    # Row 2: Add Country & Update Stock
    markup_list.append([
        a_btn("Add Country", "5397916757333654639", "admin_add_country", "success"),
        a_btn("Update Stock", "5375338737028841420", "admin_update_stock", "primary")
    ])
    
    # Row 3: User Management
    markup_list.append([a_btn("User Management", "5332531536723984111", "admin_user_mgmt", "primary")])
    
    # Row 4: Withdrawals
    markup_list.append([
        a_btn("Withdrawals", "5327864135633886155", "admin_withdrawals", "primary")
    ])
    
    # Row 5: System Settings
    markup_list.append([a_btn("System Settings", "5247121181420575838", "admin_system_settings", "primary")])
    
    # Row 6: Groups & Channels
    markup_list.append([
        a_btn("Groups", "5330237710655306682", "admin_groups", "primary"),
        a_btn("Channels", "6269303009658802514", "admin_channels", "primary")
    ])
    # Row 7: Admins & OTP Providers
    markup_list.append([
        a_btn("Admins", "6269458311381258421", "admin_admins", "primary"),
        a_btn("OTP Providers", "5287480366330816274", "admin_otp_providers", "primary")
    ])
    
    # Row 8: Backup Users & Import Users
    markup_list.append([
        a_btn("Backup Users", "4967762670104085632", "admin_backup_menu", "primary"),
        a_btn("Import Users", "5465493682574604098", "admin_import_menu", "primary")
    ])
    
    # Row 9: Ban/Unban Users
    markup_list.append([
        a_btn("Ban/Unban Users", "5212992409213872592", "admin_ban_unban", "danger")
    ])

    # Row 10: Broadcast
    markup_list.append([
        a_btn("Broadcast", "5197304993920616826", "admin_broadcast", "success")
    ])
...
async def admin_otp_providers_callback(call):
    ...
    markup = InlineKeyboardMarkup()
    markup.row(InlineKeyboardButton(f"      {status_emoji} API System Settings", callback_data="admin_api_settings", style="primary"))
    markup.row(InlineKeyboardButton("      📊 Live View SMS", callback_data="admin_live_view_sms", style="primary"))
    markup.row(InlineKeyboardButton("      Group OTP Provider — 🔴 Not Configured", callback_data="admin_group_otp", style="primary"))
    markup.row(InlineKeyboardButton("      Back to Admin", callback_data="admin_panel", style="danger"))
...
async def admin_api_settings_callback(call, answer=True):
    ...
    markup.row(InlineKeyboardButton("🔙 Back to Providers", callback_data="admin_otp_providers", style="danger"))
                ...
                if response.status == 200:
                    ...
                    if isinstance(data, list) and len(data) > 0:
                        ...
                        for sms in last_3:
                            ...
                            f_id = FLAG_IDS.get(country_iso)
                            f_html = f'<tg-emoji emoji-id="{f_id}">🌍</tg-emoji>' if f_id and str(f_id).isdigit() else "🌍"
                            ...
                            s_id = SERVICE_IDS.get(svc_name)
                            s_html = f'<tg-emoji emoji-id="{s_id}">📱</tg-emoji>' if s_id and str(s_id).isdigit() else "📱"
...
async def set_num_req_callback(call):
    ...
    f_id = FLAG_IDS.get(country_name.upper(), "5323379047315555501")
    f_icon = f'<tg-emoji emoji-id="{f_id}">🏳️</tg-emoji>'
    s_key = service_name.upper()
    s_id = SERVICE_IDS.get(s_key, "5323379047315555501")
    s_icon = f'<tg-emoji emoji-id="{s_id}">📱</tg-emoji>'
...
async def admin_admins_callback(call_or_msg):
    ...
    markup.add(InlineKeyboardButton("↩️ Back to System Edit", callback_data="admin_system_edit"))
...
async def render_edit_country(chat_id, message_id, country_id, call_id=None):
    ...
    msg_text = (
        ...
        f"🔑 ID: {c_name}\n"
        f"🎌 Flag: {c_flag}\n"
        f"⭐ Premium Emoji ID: {c_emoji_id_display}\n"
...
async def back_to_services_callback(call):
    ...
    for service_name, emoji_id in services:
        ...
        btn_icon = SERVICE_IDS.get(s_key) or (emoji_id if emoji_id and emoji_id.isdigit() else "5323379047315555501")
        ...
        if btn_icon and str(btn_icon).isdigit():
            btn_data["icon_custom_emoji_id"] = int(btn_icon)
    ...
    markup_list.append([{
        ...
        "style": "success",
        "icon_custom_emoji_id": 5215361191051798408
    }])
...
async def service_selected_callback(call):
    ...
    for c_id, c_name, c_code, c_flag, c_earn, c_stock, c_emoji_id in countries:
        ...
        btn_icon = FLAG_IDS.get(c_key) or c_emoji_id or "5433666360003540231"
    ...
    header_icon = ""
    if s_key in SERVICE_IDS:
        header_icon = f'<tg-emoji emoji-id="{SERVICE_IDS[s_key]}">📱</tg-emoji> '

    header_text = f"{header_icon}<b>Select country for {service_name}:</b>"
...
async def country_selected_callback(call):
    ...
    if s_emoji and s_emoji != "Not Set":
        if s_emoji.isdigit():
            service_emoji_html = f'<tg-emoji emoji-id="{s_emoji}">📱</tg-emoji>'
        else:
    ...
    flag_id = FLAG_IDS.get(iso_code.upper()) or c_emoji
    
    if flag_id and str(flag_id).isdigit():
        country_emoji_html = f'<tg-emoji emoji-id="{flag_id}">{flag or "🌍"}</tg-emoji>'
    else:
        country_emoji_html = flag or "🌍"
    ...
    for num in selected_numbers:
        ...
        btn_data = {
            "text": btn_text,
            "copy_text": {"text": formatted_num},
            "style": "primary" # Blue color for numbers
        }
        if flag_id and str(flag_id).isdigit():
            btn_data["icon_custom_emoji_id"] = int(flag_id)
    ...
    markup_list.append([{
        ...
        "callback_data": f"service_{service_name}", 
        "style": "success",
        "icon_custom_emoji_id": 6188045471118790922
    }])
    markup_list.append([{
        "text": "      View OTP", 
        "url": otp_link, 
        "style": "success",
        "icon_custom_emoji_id": 5841494459904168607
    }])
    markup_list.append([{
        "text": "      Back To Services", 
        "callback_data": "back_to_services", 
...
async def handle_buttons(message):
    ...
    main_menu_buttons = ["📲 Get Number", "🌎 Stock Status", "📥 Download OTP", "💳 My Balance", "💸 Withdraw", "📊 Live Traffic", "🛠 Admin Panel"]
    ...
    if text in ["📲 Get Number", "Get Number"]:
    ...
    elif text in ["🌎 Stock Status", "Stock Status", "Available Country"]:
        async with aiosqlite.connect("database.db") as db:
        ...
        for s_name, data in services_data.items():
            ...
            if data["emoji"] and data["emoji"] != "Not Set":
                if bot_mode == "Premium" and data["emoji"].isdigit():
                    s_icon = f'<tg-emoji emoji-id="{data["emoji"]}">📱</tg-emoji>'
                else:
            ...
            for item in data["items"]:
                ...
                if item["c_emoji"] and item["c_emoji"] != "Not Set":
                    if bot_mode == "Premium" and item["c_emoji"].isdigit():
                        c_icon = f'<tg-emoji emoji-id="{item["c_emoji"]}">{item["flag"] or "🌍"}</tg-emoji>'
                    elif not item["c_emoji"].isdigit():
                        c_icon = item["c_emoji"]
    ...
    elif text in ["💳 My Balance", "My Balance", "Balance"]:
        async with aiosqlite.connect("database.db") as db:
    ...
    elif text in ["📊 Live Traffic", "Live Traffic", "Status"]:
        async with aiosqlite.connect("database.db") as db:
                                    ...
                                    for sms in sms_list:
                                        ...
                                        if session:
                                            ...
                                            if s_emoji_id and s_emoji_id != "Not Set":
                                                if bot_mode == "Premium" and s_emoji_id.isdigit():
                                                    s_emoji_html = f'<tg-emoji emoji-id="{s_emoji_id}">📱</tg-emoji>'
                                                else:
                                            ...
                                            if c_emoji_id and c_emoji_id != "Not Set":
                                                if bot_mode == "Premium" and c_emoji_id.isdigit():
                                                    c_flag_html = f'<tg-emoji emoji-id="{c_emoji_id}">{flag_val or "🌍"}</tg-emoji>'
                                                elif not c_emoji_id.isdigit():
